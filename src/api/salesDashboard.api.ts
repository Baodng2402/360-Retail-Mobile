...
export const salesDashboardApi = {
  async getOverview(params?: { from?: string; to?: string }): Promise<SalesOverview> {
    ...
    const url = `sales/dashboard/overview${query.toString() ? `?${query.toString()}` : ''}`;
    const res = await apiClient.get<ApiResponse<SalesOverview> | SalesOverview>(url);
  ...
  async getRevenueChart(params?: {
    ...
    const res = await apiClient.get<ApiResponse<RevenueChartResponse> | RevenueChartResponse>(
  ...
  async getTopProducts(params?: {
    ...
    const url = `sales/dashboard/top-products${query.toString() ? `?${query.toString()}` : ''}`;
    const res = await apiClient.get<ApiResponse<TopProduct[]> | TopProduct[]>(url);
  ...
  async getOrderStatus(params?: { from?: string; to?: string }): Promise<OrderStatusOverview> {
    const query = new URLSearchParams();
    if (params?.from) query.append('from', params.from);
    if (params?.to) query.append('to', params.to);
    const url = `sales/dashboard/order-status${query.toString() ? `?${query.toString()}` : ''}`;
    const res = await apiClient.get<ApiResponse<OrderStatusOverview> | OrderStatusOverview>(url);
    if ('success' in res.data && res.data.success && res.data.data) {
      return res.data.data;
    }
  ...
  async getInventorySummary(): Promise<InventorySummary> {
    const res = await apiClient.get<ApiResponse<InventorySummary> | InventorySummary>(
  ...
  async getRecentActivity(limit = 20): Promise<RecentActivityResponse> {
    const res = await apiClient.get<ApiResponse<RecentActivityResponse> | RecentActivityResponse>(
