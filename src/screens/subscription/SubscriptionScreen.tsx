...
export function SubscriptionScreen({ navigation }: Props) {
  ...
  const formatDate = (d: string | null) => (d ? new Date(d).toLocaleDateString('vi-VN') : 'N/A');

  const handleOpenPayment = useCallback(async () => {
    const url = 'https://360retail.shop/';
    try {
      await Linking.openURL(url);
    } catch {
      Toast.show({ type: 'error', text1: 'Lỗi', text2: 'Không thể mở liên kết' });
    }
  }, []);
      ...
      <ScrollView
          ...
          <View className="mb-4 rounded-2xl bg-surface p-5">
            ...
            <View className="flex-row items-center justify-between">
              ...
              <View
                ...
                style={{
                  backgroundColor:
                    subscription.status === 'Active' ? COLORS.successLight : COLORS.warningLight,
                }}>
                <Text
            ...
            <View className="mt-3 rounded-xl bg-bg p-3">
              ...
              <InfoRow
                label="Còn lại"
                value={
                  subscription.daysRemaining !== null ? `${subscription.daysRemaining} ngày` : 'N/A'
                }
              />
            </View>
          </View>
        ...
        {plans.map((plan) => (
          <View key={plan.id} className="mb-3 rounded-2xl bg-surface p-4">
            ...
            {plan.description && (
              <Text className="mt-2 text-sm text-muted">{plan.description}</Text>
            )}
        ...
        <View className="mt-2 items-center rounded-2xl bg-surface p-5">
          ...
          <TouchableOpacity
            ...
            <Text className="font-semibold text-white">Mở màn hình thanh toán</Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
    </View>
